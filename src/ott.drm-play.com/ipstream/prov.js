...
version += ' ipstream-1118';
var provName = 'IpStream';
p_pref = 'ipst';
var url_epg = 'http://api.gazoni1.com',
    url_list = 'http://file.ipstr.im/iptv/m3u_alex-',
    url_media = 'http://www.ishara.tv/kino-ottn/',
    url_pin = 'http://file.ipstr.im/api/ott-play-apiQ3r.php';

function getProviderParams(){
    _getParams();
    ts_hls = 1;
    if((login.length < 4) || (pass.length < 6)) alert('Для доступа необходимо ввести ПИН и получить данные!');
    return (login.length >= 4) && (pass.length >= 6);
}
function setProviderParams(){
    var pin = $("#pin").val().trim();
    if(pin) getDataByPIN(pin);
    if((login.length < 4) || (pass.length < 6)) alert('Для доступа необходимо ввести ПИН и получить данные!');
    return false;
}
...
function duneAddSettings(ind){
    ...
    _getParams();
    popupArray.splice(ind, 0, provName+': PIN', provName+': Тип потоков: '+shTarr[ts_hls], provName+': Информация о подписке');
    popupDetail.splice(ind, 0, 'Получение данных по ПИНкоду из кабинета на сайте', 'Выберите тип потоков', '');
    popupActions.splice(ind, 0, doPIN, doEditType, doUserInfo);
}
